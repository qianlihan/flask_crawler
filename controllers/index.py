...
def index():
    req = request.values
    order_by_f = str(req['order']) if (
        "order" in req and req['order']) else "lastest"
    page = 1
    ...
    if order_by_f == "hot":
        query = query.order_by(Movie.gross.desc(), Movie.id.desc())
    else:
        query = query.order_by(Movie.release_date.desc(), Movie.id.desc())
